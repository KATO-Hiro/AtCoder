...
def calc(b, x, y):
    inf = 10 ** 9
    s_min = SortedMultiset([inf])
    s_max = SortedMultiset([inf])
    ...
    for i in range(size):
        value_min = s_min.ge(i)
        value_max = s_max.ge(i)

        if value_min == inf or value_max == inf:
            continue

        results += size - max(value_min, value_max)
    return results
