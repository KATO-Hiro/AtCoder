...
def main():
    ...
    for i in range(n):
        ...
        ci -= 1
        rcl.append((ri, ci, ci + li, i))

    rcl = sorted(rcl, key=lambda x: x[0], reverse=True)
    a = [h] * w

    # 区間加算・区間最大値取得と思っていたが、区間変更・区間最小値取得の方が考察・実装ともにしやすい
    inf = 10**18
    lst = LazySegTree(min, inf, min, min, inf, a)
    ans = [0] * n

    for _, left, right, i in rcl:
        hi = lst.prod(left, right)
        ans[i] = hi

        lst.apply(left, right, hi - 1)
