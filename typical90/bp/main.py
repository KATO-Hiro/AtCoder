...
def main():
    ...
    for i in range(q):
        ti, xi, yi, vi = map(int, input().split())
        xi -= 1
        yi -= 1
        t[i], x[i], y[i], v[i] = ti, xi, yi, vi
    ...
    for i in range(n - 1):
        potential[i + 1] = summed[i] - potential[i]

    for i in range(q):
        if t[i] == 0:
            uf.merge_if_needs(x[i], y[i])
        else:
            if not uf.is_same_group(x[i], y[i]):
                print("Ambiguous")
                continue

            if (x[i] + y[i]) % 2 == 0:
                print(potential[y[i]] - potential[x[i]] + v[i])
            else:
