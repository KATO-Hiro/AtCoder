...
def main():
    ...
    for _ in range(q):
        ...
        s_min, s_max = s.lt(xi), s.gt(xi)
        t_min, t_max = t.lt(xi), t.gt(xi)

        if s_min is not None and t_min is not None:
            ans = min(ans, max(abs(xi - s_min), abs(xi - t_min)))
        if s_max is not None and t_max is not None:
            ans = min(ans, max(abs(s_max - xi), abs(t_max - xi)))
        if s_min is not None and t_max is not None:
            ans = min(ans, abs(xi - s_min) + 2 * abs(t_max - xi), 2 * abs(xi - s_min) + abs(t_max - xi))
        if t_min is not None and s_max is not None:
            ans = min(ans, abs(xi - t_min) + 2 * abs(s_max - xi), 2 * abs(xi - t_min) + abs(s_max - xi))
