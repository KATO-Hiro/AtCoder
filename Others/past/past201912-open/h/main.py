...
def main():
    ...
    for _ in range(q):
        ...
        if si[0] == 1:
            ...
            if xi % 2 == 1:
                if st_odd.prod(yi, yi + 1) >= ai:
                    st_odd.apply(yi, yi + 1, -ai)
                    ans += ai
            else:
                if st_even.prod(yi, yi + 1) >= ai:
                    st_even.apply(yi, yi + 1, -ai)
                    ans += ai
        elif si[0] == 2:
            ai = si[1]

            if st_odd.prod(0, m) >= ai:
                st_odd.apply(0, m, -ai)
                ans += ai * m
        else:
            ai = si[1]

            if min(st_odd.prod(0, m), st_even.prod(0, n - m)) >= ai:
                st_odd.apply(0, m, -ai)
                st_even.apply(0, n - m, -ai)
                ans += ai * n
