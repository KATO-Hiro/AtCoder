...
def main():
    ...
    n = int(input())
    inf = 10**18
    x = [0] + list(map(int, input().split())) + [inf]

    d_min = defaultdict(int)
    d_min[x[0]] = x[1]
    d_min[inf] = inf
    ...
    for i, xi in enumerate(x[1:-1]):
        ...
        p1, p2 = s.lt(xi), s.gt(xi)
        id1, id2 = ids[p1], ids[p2]
        ...
        d_min[p1] = min(d_min[p1], abs(xi - x[id1]))
        d_min[p2] = min(d_min[p2], abs(x[id2] - xi))
        d_min[xi] = min(abs(xi - x[id1]), abs(x[id2] - xi))

        cur_sum += d_min[p1] + d_min[xi]
