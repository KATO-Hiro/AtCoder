...
def main():
    ...
    q = int(input())
    is_red = [[False] * (w + 10) for _ in range(h + 10)]
    uf = UnionFind(h * w + 20)
    dxy = [(-1, 0), (1, 0), (0, -1), (0, 1)]
    ...
    for _ in range(q):
        ...
        if qi[0] == 1:
            ...
            for dx, dy in dxy:
                ...
                if 0 <= nx <= w and 0 <= ny <= h and is_red[ny][nx]:
                    center = ri * w + ci 
                    point =  ny * w + nx
                    uf.merge_if_needs(center, point)
        else:
            rai = qi[1] - 1
            cai = qi[2] - 1
            rbi = qi[3] - 1
            ...
            if is_red[rai][cai] and is_red[rbi][cbi] and uf.is_same_group(p1, p2):
                print("Yes")
            else:
                print("No")
