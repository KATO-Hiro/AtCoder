...
def main():
    ...
    for _ in range(m):
        ...
        # 木がある = 最短経路に対するボーナス(= 距離を短くする)と考える
        # 整数倍(> 頂点数) + 木があるときは-1
        # ci: cost
        # bi: edge (0-indexed)
        edges[ai].append((ci * ratio - di, bi))
        edges[bi].append((ci * ratio - di, ai))

    dist, _ = dijkstra(vertex_count=n, source=0, edges=edges)
    dist_min = ceil(dist[n - 1] / ratio)
    tree_count = dist_min * ratio - dist[n - 1]
    print(dist_min, tree_count)
